...
VALUES
  ...
  ('Forget how to tie my shoes', now(), 3, TRUE);
...
VALUES
  ...
  ('Duis vitae sodales risus, sollicitudin..', 4, 4),
  ('Bork!', 10, 2),
  ...
  ('Dreamwork makes the team work!', 8, 5),
...
VALUES
  ...
  (2, 4, FALSE, TRUE, now() + '30 days'::INTERVAL, 'I want to feel fulfilled'),
  (7, 3, TRUE, TRUE, now() + '7 days'::INTERVAL, 'To help others and make myself a legacy'),
  (1, 3, FALSE, TRUE, now() + '7 days'::INTERVAL, 'The fame'),
  (8, 5, FALSE, FALSE, now() + '3 days'::INTERVAL, 'To break out of my comfort zone!'),
  (4, 2, TRUE, TRUE, now() + '1 days'::INTERVAL, 'For the money!'),
  (2, 3, TRUE, TRUE, now() + '7 days'::INTERVAL, 'So I can be the BEST!'),
  (6, 3, FALSE, TRUE, now() + '7 days'::INTERVAL, 'I always wanted to do this thing'),
  (4, 5, FALSE, FALSE, now() + '3 days'::INTERVAL, 'I could add the tropy to my trophy shelf'),
  (8, 3, TRUE, FALSE, now() + '7 days'::INTERVAL, 'The $$$'),
  (6, 2, FALSE, TRUE, now() + '7 days'::INTERVAL, 'Even if I don''t get it right at first I know I will learn a lot'),
  (4, 3, FALSE, FALSE, now() + '3 days'::INTERVAL, 'No reason not to try'),
  (10, 2, TRUE, TRUE, now() + '1 days'::INTERVAL, 'It sounds like a lot of fun'),
    (1, 1, TRUE, FALSE, now() + '35 days'::INTERVAL, 'This is the only thing'),
  (2, 4, FALSE, TRUE, now() + '30 days'::INTERVAL, 'Lorem ipsum'),
